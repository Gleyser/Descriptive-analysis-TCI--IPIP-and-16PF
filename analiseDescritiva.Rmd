...
```{r}
projetos <- dados %>%
  group_by(projetoAtual, iptc) %>%
  summarise(n = n()) %>% 
  mutate(iptc_projeto = n / mean(iptc)) 


projetos %>% 
  ggplot(aes(x = projetoAtual), y = iptc_projeto ) + 
  geom_col(width = .3, alpha = .5, fill = "black") +
  ggtitle("Taxa de resultados igual a zero por grupo") +
  xlab("Grupo") + 
  ylab("Taxa de resultados iguais a zero (%)")
```
